import { BigInt } from "@graphprotocol/graph-ts"
import {
  Contract,
  ActivePoolAddressChanged,
  CollSurplusPoolAddressChanged,
  DefaultPoolAddressChanged,
  GasPoolAddressChanged,
  OwnershipTransferred,
  PriceFeedAddressChanged,
  SYETIAddressChanged,
  SortedTrovesAddressChanged,
  StabilityPoolAddressChanged,
  TroveCreated,
  TroveManagerAddressChanged,
  TroveUpdated,
  TroveUpdated1,
  YUSDBorrowingFeePaid,
  YUSDTokenAddressChanged
} from "../generated/Contract/Contract"
import { ExampleEntity } from "../generated/schema"

export function handleActivePoolAddressChanged(
  event: ActivePoolAddressChanged
): void {
  // Entities can be loaded from the store using a string ID; this ID
  // needs to be unique across all entities of the same type
  let entity = ExampleEntity.load(event.transaction.from.toHex())

  // Entities only exist after they have been saved to the store;
  // `null` checks allow to create entities on demand
  if (!entity) {
    entity = new ExampleEntity(event.transaction.from.toHex())

    // Entity fields can be set using simple assignments
    entity.count = BigInt.fromI32(0)
  }

  // BigInt and BigDecimal math are supported
  entity.count = entity.count + BigInt.fromI32(1)

  // Entity fields can be set based on event parameters
  entity._activePoolAddress = event.params._activePoolAddress

  // Entities can be written to the store with `.save()`
  entity.save()

  // Note: If a handler doesn't require existing field values, it is faster
  // _not_ to load the entity from the store. Instead, create it fresh with
  // `new Entity(...)`, set the fields that should be updated and save the
  // entity back to the store. Fields that were not set or unset remain
  // unchanged, allowing for partial updates to be applied.

  // It is also possible to access smart contracts from mappings. For
  // example, the contract that has emitted the event can be connected to
  // with:
  //
  // let contract = Contract.bind(event.address)
  //
  // The following functions can then be called on this contract to access
  // state variables and other data:
  //
  // - contract.BOOTSTRAP_PERIOD(...)
  // - contract.BORROWING_FEE_FLOOR(...)
  // - contract.CCR(...)
  // - contract.DECIMAL_PRECISION(...)
  // - contract.MCR(...)
  // - contract.MIN_NET_DEBT(...)
  // - contract.NAME(...)
  // - contract.PERCENT_DIVISOR(...)
  // - contract.REDEMPTION_FEE_FLOOR(...)
  // - contract.YUSD_GAS_COMPENSATION(...)
  // - contract._100pct(...)
  // - contract._110pct(...)
  // - contract.activePool(...)
  // - contract.defaultPool(...)
  // - contract.getCompositeDebt(...)
  // - contract.getEntireSystemColl(...)
  // - contract.getEntireSystemDebt(...)
  // - contract.isOwner(...)
  // - contract.owner(...)
  // - contract.sYETI(...)
  // - contract.sYETIAddress(...)
  // - contract.sortedTroves(...)
  // - contract.troveManager(...)
  // - contract.yusdToken(...)
}

export function handleCollSurplusPoolAddressChanged(
  event: CollSurplusPoolAddressChanged
): void {}

export function handleDefaultPoolAddressChanged(
  event: DefaultPoolAddressChanged
): void {}

export function handleGasPoolAddressChanged(
  event: GasPoolAddressChanged
): void {}

export function handleOwnershipTransferred(event: OwnershipTransferred): void {}

export function handlePriceFeedAddressChanged(
  event: PriceFeedAddressChanged
): void {}

export function handleSYETIAddressChanged(event: SYETIAddressChanged): void {}

export function handleSortedTrovesAddressChanged(
  event: SortedTrovesAddressChanged
): void {}

export function handleStabilityPoolAddressChanged(
  event: StabilityPoolAddressChanged
): void {}

export function handleTroveCreated(event: TroveCreated): void {}

export function handleTroveManagerAddressChanged(
  event: TroveManagerAddressChanged
): void {}

export function handleTroveUpdated(event: TroveUpdated): void {}

export function handleTroveUpdated1(event: TroveUpdated1): void {}

export function handleYUSDBorrowingFeePaid(event: YUSDBorrowingFeePaid): void {}

export function handleYUSDTokenAddressChanged(
  event: YUSDTokenAddressChanged
): void {}
